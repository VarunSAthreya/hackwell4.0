// NOTE: CODE IS COMPLICATED AND COMPLETELY COPIED. DO NOT TOUCH ANYTHING.

// UTILS
export const utils = (w) => {
    /**
    ...
    utils.randMultiple = function (multiple, range) {
        return (
            Math.floor(
                (Math.random() * (range - multiple) + multiple + 1) / multiple
            ) * multiple
        );
    };
        ...
        if (xDiff < 4 && yDiff < 4) {
            for (var pixelX = xMin; pixelX < xMax; pixelX++) {
                for (var pixelY = yMin; pixelY < yMax; pixelY++) {
                    if (
                        ...
                        pixels2[(pixelX - x2 + (pixelY - y2) * w2) * 4 + 3] !==
                            0
                    ) {
                        return true;
                    }
            ...
            for (var offsetY = 0; offsetY < incY; offsetY++) {
                for (var offsetX = 0; offsetX < incX; offsetX++) {
                    for (
                        var pixelY = yMin + offsetY;
                        pixelY < yMax;
                        pixelY += incY
                    ) {
                        for (
                            var pixelX = xMin + offsetX;
                            pixelX < xMax;
                            pixelX += incX
                        ) {
                            if (
                                pixels[
                                    (pixelX - x + (pixelY - y) * w) * 4 + 3
                                ] !== 0 &&
                                pixels2[
                                    (pixelX - x2 + (pixelY - y2) * w2) * 4 + 3
                                ] !== 0
                            ) {
                                return true;
                            }
...
export const canvas = (w, $) => {
    ...
    var utils = w.utils,
        ...
        fir_balls = [],
        numOfBalls = 500,
        colors = ['#F7C758', '#6B1DCB', '#FD3058', '#FE9B58'],
        speed = 0.001,
        trailLength = 0,
        /**
    ...
    if ($canvas.width > 512) $canvas.width -= 20;
    ...
    function generateVelocity(ball, index) {
        ...
        var hue =
            ((ball.range - 1) * (300 - 200)) / ((bottom - top) / 2 - 1) + 140;
    ...
    (function animate() {
        ...
        if (isMouseDown && trailLength < 95) {
            trailLength += 5;
        } else if (!isMouseDown && trailLength > 0) {
            trailLength -= 0.5;
        }

        if (isMouseDown && speed < 0.005) {
            speed += 0.0005;
        } else if (!isMouseDown && speed > 0.001) {
            speed -= 0.00005;
        }
